...
public class EntityHighEnergyParticleFlow extends EntitySummonedSwordBase {
    public EntityHighEnergyParticleFlow(World par1World) {
        super(par1World);
    }

    public EntityHighEnergyParticleFlow(World par1World, EntityLivingBase entityLiving, float AttackLevel) {
        super(par1World, entityLiving, AttackLevel);
    }

    public EntityHighEnergyParticleFlow(World par1World, EntityLivingBase entityLiving, float AttackLevel, float roll) {
        super(par1World, entityLiving, AttackLevel, roll);
    }

    private static final DataParameter<Boolean> Burst = EntityDataManager.<Boolean>createKey(EntityHighEnergyParticleFlow.class, DataSerializers.BOOLEAN);
    @Override
    ...
    protected void attackEntity(Entity target) {
        ...
        if(!this.world.isRemote){
            ...
            if(!blade.isEmpty() && target instanceof EntityLivingBase && thrower != null && thrower instanceof EntityLivingBase){
                ...
                if(!getBurst())
                    ((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.POISON, 20 * 5, 1));
